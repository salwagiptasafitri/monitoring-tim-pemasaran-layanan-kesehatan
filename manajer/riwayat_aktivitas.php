...
/* FILTER BULAN DAN TAHUN DI SEBELAH CARI */
.filter-tanggal-wrapper {
    display: inline-flex;
    align-items: center;
    gap: 6px;
    margin-right: 12px;
...
<tr class="filter-row">
    ...
    <th>
        <select class="form-control form-control-sm column-filter">
            <option value="">Semua Produk/Jasa</option>
        </select>
    </th>
    <th>
        <select class="form-control form-control-sm column-filter">
...
$(document).ready(function () {
    ...
    var table = $('#datatablesSimple').DataTable({
        ...
        initComplete: function () {
            ...
            api.columns().every(function (index) {
                var column = this;
                var select = $('.filter-row th').eq(index).find('select');
    ...
    var filterHtml = `
        <div class="filter-tanggal-wrapper">
    ...
    $('#datatablesSimple_filter').prepend(filterHtml);


    // Ambil tahun otomatis dari kolom Tanggal
    var tahunList = [];

    table.column(3).data().each(function (value) {
        var tanggal = $('<div>').html(value).text().trim();
    ...
    // Custom filter bulan dan tahun
    $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        var bulanDipilih = $('#filterBulan').val();
        var tahunDipilih = $('#filterTahun').val();

        var tanggal = data[3]; // kolom Tanggal
        var pecah = tanggal.split('-');
        ...
        var bulanData = pecah[1];
        var tahunData = pecah[2];

        if (bulanDipilih !== '' && bulanData !== bulanDipilih) {
            return false;
        }

        if (tahunDipilih !== '' && tahunData !== tahunDipilih) {
            return false;
        }
